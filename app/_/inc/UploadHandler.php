...
class UploadHandler
{
    ...
    protected function set_additional_file_properties($file) {
        $file->deleteUrl = $this->options['script_url']
            .$this->get_query_separator($this->options['script_url'])
            .$this->get_singular_param_name()
            .'='.rawurlencode($file->fid)
            ."&dtoken="
            .rawurlencode($file->dtoken);
    ...
    protected function generateFileId() {
        $chars = array_merge(range('a', 'z'), range(0, 9));
        shuffle($chars);
        return implode(array_slice($chars, 0,32));
    }

    protected function generateDeleteToken() {
        $chars = array_merge(range('a', 'z'), range(0, 9));
        shuffle($chars);
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $file->name = $this->get_file_name($uploaded_file, $name, $size, $type, $error,
            $index, $content_range);
        $file->fid = $this->generateFileId() ;
        $file->dtoken = $this->generateDeleteToken() ;
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $file_path = $this->get_upload_path($file->fid);
            ...
            if ($file_size === $file->size) {
                $file->url = $this->get_download_url($file->fid);
            ...
            $nfile->type = $file->type ;
            $nfile->size = $file->size ;
            $nfile->dtoken = $file->dtoken ;
            $nfile->adddate = $datenow ;
            $nfile->lastusagedate = $datenow ;
            $nfile->usage = 0 ;
